...
ImageFactory::~ImageFactory() {
  ...
  std::map<std::string, std::vector<SDL_Texture*>>::iterator textures = multiTextures.begin();
  ...
  std::map<std::string, std::vector<Image*>>::iterator images = multiImages.begin();
  while (images != multiImages.end()) {
...
std::vector<Image*> ImageFactory::getImages(const std::string& name, const std::string& direction) {
  // First search map to see if we've already made it:
  std::map<std::string, std::vector<Image*> >::const_iterator
    pos = multiImages.find(name + direction);
  if ( pos != multiImages.end() ) {
    return pos->second;
  }
  ...
  IoMod& iomod = IoMod::getInstance();
  RenderContext& renderContext  = RenderContext::getInstance();
  std::string sheetPath = gdata.getXmlStr(name+ "/file" + direction);
  SDL_Surface* spriteSurface = iomod.readSurface(sheetPath);
  bool transparency = gdata.getXmlBool(name+"/transparency");
  ...
  multiSurfaces[name + direction] = surfaces;
  multiTextures[name + direction] = textures;
  multiImages[name + direction] = images;
  return images;
}
